...
output (with taxonomy IDs of -1 for consistency with MEGAN).',
	...
	(
		'Default:',
		'<queryID> <taxName> <taxID> <score>',
		'Summarized (-s):',
		'<count> <taxName> <taxID> <score>'
	);
	print "\n";
	printColumns
	(
		...
		'   taxName',
'The NCBI taxonomy Name the query was assigned to (or -1 if it has no hits).',
		'   taxID',
...
if ( getOption('summarize') )
{
	my %magnitudes;
	my %totalScores;
	print OUT "#count\ttaxName\ttaxID\t$scoreName\n";
...
{
	print OUT "#queryID\ttaxName\ttaxID\t$scoreName\n";
	
	foreach my $queryID ( sort keys %taxIDs )
	{
                my $taxID = $taxIDs{$queryID};
                my $taxName = getTaxName($taxID);
		print OUT "$queryID\t$taxName\t$taxID\t$scores{$queryID}\n";
	}
}
