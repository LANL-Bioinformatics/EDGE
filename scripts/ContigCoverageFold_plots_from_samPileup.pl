...
#plot(1:length(a\$V1),a[,1],type=\"l\",col=\"blue\",cex=2,xlab=\"Contigs\",ylab=\"Coverage fold (x)\",main=\"\",log=\"y\")
#pa<-par(\'usr\');
...
plot(a\$Length/1000,a\$Avg_fold,pch=8,xlab=\"Contigs Length (kbp)\",ylab=\"Average Coverage Fold (x)\", main=\"Contigs Average Fold Coverage vs. Contigs Length\",log=\"y\")
...
plot(a\$Length,a\$Base_Coverage,pch=8,xlab=\"Contigs Length (bp)\",ylab=\"Contigs Coverage (%)\",main=\"Contigs Coverage vs. Contigs Length\")
...
# Contigs average fold coverage vs. GC
#png(filename=\"$outDir/${file_name}_gc_vs_avg_fold.png\",height=1024,width=1024,type=\"cairo\");
scale<-a\$Length/sd(a\$Length)
scale[scale<0.2]=0.2
plot(a\$Avg_fold,a\$GC,ylab=\"GC (%)\",xlab=\"Average Coverage fold (x)\", main=\"Contigs Average Fold Coverage vs. GC\",pch=20,col=\"blue\",cex=scale,log=\"x\");
grid(col=\"grey\")
