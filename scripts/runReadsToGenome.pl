...
if ($file_long and $paired_files and $singleton){
  `samtools merge -h $outDir/paired$$.bam -@ $samtools_threads $bam_output $outDir/paired$$.bam $outDir/singleton$$.bam $outDir/LongReads$$.bam`;
}
elsif($file_long and $paired_files)
{
  `samtools merge -h $outDir/paired$$.bam -@ $samtools_threads $bam_output $outDir/paired$$.bam $outDir/LongReads$$.bam`;
}
elsif($paired_files and $singleton)
{
  `samtools merge -h $outDir/paired$$.bam -@ $samtools_threads $bam_output $outDir/paired$$.bam $outDir/singleton$$.bam`;
}
elsif($singleton and $file_long)
{
  `samtools merge -h $outDir/singleton$$.bam -@ $samtools_threads $bam_output $outDir/singleton$$.bam $outDir/LongReads$$.bam`;
}
elsif($paired_files)
{
...
`samtools mpileup -ugf $ref_file $bam_output | bcftools view -bcg - > $bcf_output 2>/dev/null`;
`bcftools view $bcf_output 2>/dev/null | bcftools view -v -S - 2>/dev/null | vcfutils.pl varFilter -d10 -D10000 > $vcf_output`; 
}
...
open (IN,"$pileup_output") or die "$! no $pileup_output";
my %base_hash;
while (<IN>)
{
 chomp;
...
print Rscript "pdf(file=\"$plotsPdf\",width=10,height=8); \n";
...
# histgram
# read file
a<-read.table(file=\"$coverage_output\")
mean_cov<-mean(a\$V2)
std_cov<-sd(a\$V2)
b<-round (5*std_cov)
...
{
# init device
#png(filename=\"$histogram\",height=800,width=800)
#bitmap(file=\"$histogram\",width=1024,height=640,units=\"px\");

plot(h\$count[2:length(h\$count)],type=\'h\',lwd=3, col=\'black\',main=\"${prefix}_${ref_name}\",xlab=\"Coverage(fold)\",ylab=\'Frequency\',xaxt=\"n\",xlim=c(1,length(h\$count)))
...
# coverage plot
# init device
#png(filename=\"$coverage_plot\",width=1024,height=640)
#bitmap(file=\"$coverage_plot\",width=1024,height=640,units=\"px\");
def.par <- par(no.readonly = TRUE) # get default parameters

# setup plotting area
#par(mar = c(5, 5, 5, 5), xpd=TRUE, cex.main=1.2, cex.lab=1.2, cex.axis=1.2)
a<-read.table(file=\"$WindowCoverage_output\")
refGC<-read.table(file=\"$ref_window_gc\")
refGC_coord<-refGC\$V2[refGC\$V1==\"$ref_name\"]
refGC_percetage<-refGC\$V3[refGC\$V1==\"$ref_name\"]
data.gaps<-read.table(file=\"$gap_output\",header=TRUE)
gapBp<-sum(data.gaps\$Length);
gapNum<-length(data.gaps\$Length);

par(fig=c(0,1,0,0.75),mar=c(5, 4, 1, 2),cex.main=1.2)
plot(a\$V1,a\$V2,type=\"l\",col=\"blue\",cex=2,xlab=\"$ref_name\",ylab=\"Coverage (fold)\",main=\"\",xlim=c(0,$ref_len))
leg.txt<-c(coverage,paste(\"Average fold: \",format(mean_cov,digit=4),\"sd\",format(std_cov,digit=4)),paste(\"# of Gaps: \",gapNum),paste(\"Gap bp: \", gapBp))
if ($no_snp == 0)
{
...
if (gapNum > 0){
  for(i in 1:dim(data.gaps)[1]){
    rect(data.gaps[i,1],round(pa[3]),data.gaps[i,2],round(pa[3])+(round(pa[4])-round(pa[3]))/100,col=\"red\",border=NA)
  }
}
par(fig=c(0,1,0.75,1),mar=c(0, 4, 2, 2),new=TRUE,cex.main=1.2)
plot(refGC_coord,refGC_percetage,type=\"l\",cex=2,xaxt=\'n\',xlab=\"\",ylab=\"GC %\",main=\"\",xlim=c(0,$ref_len),bty='n')
...
"
par(mar = c(5, 5, 5, 5), xpd=TRUE, cex.main=1.2, cex.lab=1.2, cex.axis=1.2)
bitmap(file=\"$other_coverage_plot\",width=1024,height=640,units=\"px\");
#png(filename=\"$other_coverage_plot\",width=1024,height=640)
properC<-read.table(file=\"$properpair_coverage_output\")
unproperC<-read.table(file=\"$unproperpair_coverage_output\")
properCymax<-max(properC\$V2)
if (properCymax==0) {properCymax <- 1}
plot(properC\$V1,properC\$V2,type=\'l\',xlab=\"$ref_name\",ylab=\"Coveragae Depth (fold)\",ylim=c(0,properCymax))
par(new=T)
unproperCymax<-max(unproperC\$V2)
if (unproperCymax==0) {unproperCymax <- 1}
plot(unproperC\$V1,unproperC\$V2,type=\'l\',xlab=\"\",ylab=\"\",xaxt=\'n\',yaxt=\'n\',col=\"blue\",ylim=c(0,unproperCymax))
leg.txt<-c(\"proper_paired_mapped_reads\",\"nonproper_paired_mapped_reads\")
legend(\"topright\",leg.txt,col=c(\'black\',\'blue\'),lwd=1)
