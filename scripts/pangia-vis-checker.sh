...
if [[ $(cat $BOKEH_LOG | grep -E 'NoneType|No active exception') ]]; then
	...
	nohup ${EDGE_HOME}/thirdParty/Mambaforge/bin/bokeh serve pangia-vis --port 5100 --allow-websocket-origin '*' &> ${EDGE_HOME}/edge_ui/bokeh.log &
	#nohup ${EDGE_HOME}/thirdParty/Mambaforge/bin/bokeh serve pangia-vis --port 5100 --keep-alive 10000 --allow-websocket-origin '*' --use-xheaders &> ${EDGE_HOME}/edge_ui/bokeh.log &
else
	DATETIME=`date "+%x %H:%M:%S"`
