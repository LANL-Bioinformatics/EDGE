...
function paf_call(args)
{
	...
	var c, min_cov_len = 10000, min_var_len = 50000, gap_thres = 50, gap_thres_long = 1000, min_mapq = 5, amb=false;
	var fa_tmp = null, fa, fa_lens, is_vcf = false, sample_name = "sample";
	while ((c = getopt(args, "l:L:g:q:B:f:s:n")) != null) {
	...
	if (args.length == getopt.ind) {
		...
		print("  -n        call a SNP if there is an ambiguous base");
		exit(1);
	}
			...
			while ((m = re_cs.exec(cs)) != null) {
				...
				if (m[1] == '=' || m[1] == ':') {
					...
					if (amb){
						...
						++n_diff;
					}else if (br != 'n' && bq != 'n') { // don't call a SNP if there is an ambiguous base
						out.push([t[5], x, x+1, cov, t[11], br, bq, query, qs, qe, rev? '-' : '+']);
						++n_diff;
					}
					++x, ++blen;
