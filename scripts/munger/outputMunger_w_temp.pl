...
sub pull_binning {
	...
	my $bin_log = "$output_dir/${proj_realname}_bin.log";
        my (@errs,$errs);
        @errs = `grep stop $bin_log` if ( -e "$bin_log");
        $errs = join ("<br/>\n",@errs) if (@errs);

        $vars->{ERROR_Binning}   = "$errs" if ($errs);

	my $bin_json_summary_result = "$output_dir/${proj_realname}_bin.summary.json";

	my $binned_count=0;
	if ( -e $bin_summary_result){ 
...
sub pull_readmapping_ref {
	...
	#	if ($_ =~ /^(\d+) \+ \d+ mapped/) 		{ $vars->{RMREFMAPPED} = $1; $vars->{RMREFUNMAPPED} = $vars->{RMREFUSED} - $vars->{RMREFMAPPED}; next; }
		...
		if( scalar @temp > 8){
			...
			if ( -e $consensus_indels_report and ! -e $consensus_indels_report_json){
				system("perl", "$RealBin/../tab2Json_for_dataTable.pl","-out",$consensus_indels_report_json,$consensus_indels_report);
			}
			if ( -e $consensus_SNPs_report and ! -e $consensus_SNPs_report_json){
	...
	$vars->{RMREF_UM_DIR} = 1 if  -d "$out_dir/ReferenceBasedAnalysis/UnmappedReads";
...
sub parse_lineage{
	...
	while(<$fh>){
		...
		if ($lineage_assign =~ /B.1.617.2/ or $lineage_assign =~ /AY./ or $lineage_assign =~ /B.1.1.529/ or $lineage_assign =~ /BA./ ){
			$note = "; VOC";
			$lineage{$cid}->{warning} = "VOC";
		}
