...
my ($numSNPs,$numINDELs)=&SNP_INDEL_COUNT("$prefix.snps");
...
print OUT "ID\tLength\tGC(%)\tAvg_fold(x)\tBase_Coverage(%)\n";
my $unUsedRef=0;
...
foreach my $ref (sort keys %reference)
{
  ...
  print Rplot <<RCODE;
# read the data
data.hits    <- read.table(file="${prefix}.coordsR", header=TRUE, dec=".")
data.gaps    <- read.table(file="${prefix}_ref_zero_cov_coord.txt")
# variable
data.dimx <- $ref_len
...
close OUT6;
  ...
  printf  $log_fh ("Reference_Coverage:\t%.4f%%\n",$coverage);
...
sub getSeqInfo 
{
    ...
    my ($id, $seq, %hash, $GC_num, $GC_content,$seq_len);
...
sub SNP_INDEL_COUNT 
{
    ...
    return ($SNPs_count, $Indels_count);
}
