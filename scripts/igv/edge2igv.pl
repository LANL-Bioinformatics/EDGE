...
$opt{'igv-template'}           ||= "$Bin/igv.html.tmpl";
...
sub main {
	...
	copy("$Bin/../../edge_ui/javascript/igv.min.js", "$opt{outdir}/");
	...
	if( -s $opt{'in-ctg-fa'} ){
		my %contig_dict;
		$contig_dict{reference}->{id}=$opt{'projname'};
		print "\n# Preparing files for IGV with CONTIG-based tracks:\n";
		executeCommand("rm -rf $opt{'out-ctg-coord-dir'}");
		executeCommand("mkdir -m 777 -p $opt{'out-ctg-coord-dir'}");
		&generateEdgeAnalysisGff3_contig( "$opt{'out-ctg-coord-dir'}/edge_analysis_merged.gff3" );
		...
		if( -e $opt{'in-ctg-anno-gff3'} ){
			...
			unlink "$opt{outdir}/contig_annotation_tmp.gff3";
			unlink "$opt{outdir}/contig_annotation.gff3" ;
			print "Done.\n";
		}
		...
		$esc_str =~ s/\//\\\//g;
		$opt{'in-ctg-fa'} =~ s/$opt{'proj_outdir'}/..\/../;
		...
		if (-e $opt{'in-orf-ar-gff3'}){
			...
			$opt{'in-orf-ar-gff3'} =~ s/$opt{'proj_outdir'}/..\/../;
			executeCommand("ln -s $opt{'in-orf-ar-gff3'} $opt{'out-ctg-coord-dir'}/edge_ar_annotation.gff3");
			my %track=('name'=>'AR_Annotation',
				   'format'=>'gff3', 
			  	 'displayMode' => 'expanded', 
			  	 'height'=> 100, 
				 'type'=> 'annotation',
		...
		if (-e $opt{'in-orf-vf-gff3'}){
			...
			$opt{'in-orf-vf-gff3'} =~ s/$opt{'proj_outdir'}/..\/../;
			executeCommand("ln -s $opt{'in-orf-vf-gff3'} $opt{'out-ctg-coord-dir'}/edge_vf_annotation.gff3");
			my %track=('name'=>'VF_Annotation',
				   'format'=>'gff3', 
			  	 'displayMode' => 'expanded', 
			  	 'height'=> 100, 
				 'type'=> 'annotation',
		...
		if (  -s $opt{'in-ctg-fa'} < $repeat_track_size_cutoff ){
			...
			executeCommand("get_repeat_coords.pl -o $opt{'out-ctg-coord-dir'}/contigs_repeats.txt $opt{'out-ctg-coord-dir'}/contigs.fa > $opt{'out-ctg-coord-dir'}/contigs_repeats.log  2>&1");
			my %track=('name'=>'Repeat',
				   'format'=>'gff3', 
			  	 'displayMode' => 'expanded', 
			  	 'height'=> 100, 
				 'type'=> 'annotation',
		...
		if( -e $opt{'in-read2ctg-bam'} ){
			...
			$opt{'in-read2ctg-bam'} =~ s/$opt{'proj_outdir'}/..\/../;
			executeCommand("ln -s $opt{'in-read2ctg-bam'} $opt{'out-ctg-coord-dir'}/readsToContigs.sort.bam");
			executeCommand("samtools index $opt{'out-ctg-coord-dir'}/readsToContigs.sort.bam");
			my %track=('name'=>'Alignment',
				  'type'=>'alignment',
				   'format'=>'bam', 
			  	 'displayMode' => 'squished', 
			  	 'height'=> 500,
				 'squishedRowHeight'=>10, 
			  	 'url' =>'./readsToContigs.sort.bam',
				 'indexURL' => './readsToContigs.sort.bam.bai');
			push @{$contig_dict{tracks}}, \%track;
			print "Done.\n";
		}
		...
		if( -e $opt{'in-primer2ctg-bam'} ){
			...
			my %track=('name'=>'Primer Validation',
				  'type'=>'alignment',
				   'format'=>'bam', 
			  	 'displayMode' => 'squished', 
			  	 'height'=> 100,
				 'squishedRowHeight'=>10, 
			  	 'url' =>'./pcrContigValidation.sort.bam',
				 'indexURL' => './pcrContigValidation.sort.bam.bai');
			push @{$contig_dict{tracks}}, \%track;
			print "Done.\n";
		}
		my $contig_jgv_options = encode_json(\%contig_dict);
		executeCommand("sed -e 's;%OPTIONS%;$contig_jgv_options;' $opt{'igv-template'} > $opt{'out-ctg-coord-dir'}/igv.html");
	...
	if( -e $opt{'in-ref-fa'} ){
		my %ref_dict;
		$ref_dict{reference}->{id}=$opt{'projname'};
		my ($ref_name,$ref_file_num,$ref_files)=&pull_referenceName($opt{'proj_outdir'});
		print "\n# Preparing files for IGV with REF-based tracks:\n";
		executeCommand("rm -rf $opt{'out-ref-coord-dir'}/");
		executeCommand("mkdir -m 777 -p $opt{'out-ref-coord-dir'}/");
		#converting analysis to GFF3
		print "\n# Converting analysis to GFF3:\n";
		&generateEdgeAnalysisGff3_ref( "$opt{'out-ref-coord-dir'}/edge_analysis_merged.gff3" );
		...
		$opt{'in-ref-fa'} =~ s/$opt{'proj_outdir'}/..\/../;
		executeCommand("ln -s $opt{'in-ref-fa'} $opt{'out-ref-coord-dir'}/reference.fa");
		executeCommand("samtools faidx $opt{'out-ref-coord-dir'}/reference.fa");
		...
		if (  -s $opt{'in-ref-fa'} < $repeat_track_size_cutoff && $ref_file_num<2){
			print "#  - Adding repeats tracks...";
			executeCommand("get_repeat_coords.pl -o $opt{'out-ref-coord-dir'}/ref_repeats.txt  $opt{'out-ref-coord-dir'}/reference.fa >  $opt{'out-ref-coord-dir'}/ref_repeats.log  2>&1" );
				...
				my %track=('name'=>'Primer Validation',
					  'type'=>'alignment',
				  	  'format'=>'bam', 
			  	 	  ...
			  	 	  'height'=> 100,
				 	  'squishedRowHeight'=>10, 
			  	 	  'url' =>'./pcrRefValidation.sort.bam',
				 	  'indexURL' => './pcrRefValidation.sort.bam.bai');
				push @{$ref_dict{tracks}}, \%track;
				print "Done.\n";
			}
			else{
		...
		if( -e $opt{"in-ctg2ref-coords"} || -e $opt{"in-ctg2ref-snps"} || -e $opt{"in-ctg2ref-indels"}){
			...
			my %track=('name'=>'ContigToRef_Annotation',
				   'format'=>'gff3', 
			  	 'displayMode' => 'expanded', 
			  	 'height'=> 120, 
				 'type'=> 'annotation',
		...
		if ( -e $opt{"in-primer2ref-bed"} ){
			...
			$opt{'in-primer2ref-bed'} =~ s/$opt{'proj_outdir'}/..\/../;
			executeCommand("ln -s $opt{'in-primer2ref-bed'} $opt{'out-ref-coord-dir'}/primer_scheme.bed");
			my %track=('name'=>'Amplicon Primer Scheme',
				   'format'=>'bed', 
			  	 'displayMode' => 'expanded', 
				 'type'=> 'annotation',
		...
		if( -d $opt{'in-read2ref-dir'} ){
			#my @refseqs_id = keys %{$ref_name};
			foreach my $file_prefix (@$ref_files){
			#foreach my $acc ( @refseqs_id ){
				...
				if( $mapped_num ){
					...
					$bam =~ s/$opt{'proj_outdir'}/..\/../;
					executeCommand("ln -s $bam $opt{'out-ref-coord-dir'}/$file_prefix.sort.bam");
					executeCommand("samtools index $opt{'out-ref-coord-dir'}/$file_prefix.sort.bam");
					...
					my %track=('name'=>"$file_prefix Alignment",
				  		'type'=>'alignment',
				 		'format'=>'bam', 
					  	'displayMode' => 'squished', 
			  			'height'=> 500,
						'squishedRowHeight'=>10, 
			  	 		'url' =>"./$file_prefix.sort.bam",
						'indexURL' => "./$file_prefix.sort.bam.bai");
					push @{$ref_dict{tracks}}, \%track;
					print "Done.\n";
					...
					print STDERR "ReadsToRef: No mapped reads to $file_prefix. Skip converting BAM to tracks.\n";
				}
				#if ( -e $opt{'in-read2ref-vcf'} ){
				if ( -e $vcf ){
					print "#  - Adding read2ref $file_prefix VCF track...";
					...
					my %track=('name'=>"EC-19 Consensus Variants",
			  			   ...
			  			   'url' =>"./$file_prefix.vcf.gz",
				                   'indexURL' => "./$file_prefix.vcf.gz.csi");
					push @{$ref_dict{tracks}}, \%track;
					print "Done.\n";
				}
			}
		...
		if ($opt{'deltacron'}){
			...
			push @{$ref_dict{tracks}}, \%variants_check;
		}
...
sub generateEdgeAnalysisGff3_contig {
	my $outfile = shift;
...
sub generateEdgeAnalysisGff3_ref {
	my $outfile = shift;
	open GFF3OUT, ">$outfile" or die "$!";
...
[OPTIONS]
    ...
    --igv-template            \$EDGE_HOME/script/igv/igv.html.tmpl
