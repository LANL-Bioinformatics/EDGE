...
Options: -Q INT    minimum RMS mapping quality for SNPs [$opts{Q}]
         ...
         -r INT    minimum ratio of alternate bases [$opts{r}]
	 ...
	 -e FLOAT  min P-value for HWE (plus F<0) [$opts{e}]
         -I FLOAT  Maximum fraction of reads supporting an indel[$opts{I}]
...
\n/) if (@ARGV == 0 && -t STDIN);
  ...
  while (<>) {
	...
	my $flt = 0;
	# parse annotations
	my ($dp, $mq, $dp_alt, $dp_alt_ratio) = (-1, -1, -1, 0);
	if ($t[7] =~ /DP4=(\d+),(\d+),(\d+),(\d+)/i) {
	  $dp = $1 + $2 + $3 + $4;
	  $dp_alt = $3 + $4;
          $dp_alt_ratio = $dp_alt/$dp;
	}
	if ($t[7] =~ /DP=(\d+)/i) {
	  $dp = $1;
	}
	...
	$flt =11 if ($flt ==0 && $dp_alt_ratio > 0 && $dp_alt_ratio < $opts{r});
	$flt = 1 if ($flt == 0 && $mq >= 0 && $mq < $opts{Q});
	$flt =10 if ($flt == 0 && /IMF=(\d+\.\d+)/ && $1 < $opts{I});
	$flt = 7 if ($flt == 0 && /PV4=([^,]+),([^,]+),([^,]+),([^,;\t]+)/
...
sub varFilter_aux {
  ...
  if ($first->[1] == 0) {
	...
	print STDERR join("\t", substr("UQdDaGgPMSIr", $first->[1], 1), @$first[3 .. @$first-1]), "\n";
  }
}
