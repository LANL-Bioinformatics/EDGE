...
if (!$no_snp){ 
   ...
   `bcftools view $bcf_output | bcftools view -v -S - | vcfutils.pl varFilter -d10 -D10000 > $vcf_output`; 
}
...
if (!$no_snp){$stats_print_string .= "\tNaum_of_SNPs\tNum_of_INDELs";}
$stats_print_string .="\n";
...
# for coverage plot
coverage<-(length(a\$V2[a\$V2>0])/length(a\$V2))*100
if (mean_cov > 1)
{
# init device
...
leg.txt<-paste(\"Average fold: \",format(mean_cov,digit=4),\"±\", format(std_cov,digit=4));
...
#coverage<-(length(a\$V2[a\$V2>0])/length(a\$V2))*100
leg.txt<-c(paste(\"Coverage: \",format(coverage,digit=4),\"%\"),paste(\"Average fold: \",format(mean_cov,digit=4),\"±\",format(std_cov,digit=4)),paste(\"# of Gaps: \",gapNum),paste(\"Gap bp: \", gapBp))
if ($no_snp == 0)
{
