...
h$file_index<-hist(file$file_index\$Len,col=\'blue\',breaks=max(file$file_index\$Len)/100, main=\"Length Distribution\",xlab=\"Length (bp)\",sub=\"$file\");
...
hist(GC,breaks=c(0:100),xlab=\"GC (%)\",ylab=\"# of reads\",main=\"GC Histogram\",sub=\"$file\");
GCmean<-sprintf(\"Mean %.2f %%\", mean(GC))
GCsd<-sprintf(\"Std %.2f\",sd(GC))
legend(\"topleft\",legend=c(GCmean,GCsd),inset=0.02)
";
...
plot(LenCumSum0/1000000,xlab=\"# of Contig\",ylab=\"Size (Mbp)\",type=\"l\",main=\"Cumulative Length\",lwd=2)
mtext(side=3,\"Contigs are ordered from largest to smallest.\",adj=0,cex=0.7)
grid(col=\"grey\")

# Nx plot
plot(0:100,NxVector0/1000,type=\"l\",xlab=\"Nx\",ylab=\"Size (Kbp)\", main=\"Nx Length\",lwd=2)
grid(col=\"grey\")
...
# Comparison plots
# Cumulative Length plot
plot(LenCumSum$maxSeqNumIndex/1000000,xlab=\"# of Contig\",ylab=\"Size (Mbp)\",type=\"n\",main=\"Cumulative Length\",ylim=c(0,$maxTotalLen/1000000))
mtext(side=3,\"Contigs are ordered from largest to smallest.\",adj=0,cex=0.7)
grid(col=\"grey\")
...
# Nx plot 
plot(0:100,NxVector$maxTotalLenIndex/1000,type=\"n\",xlab=\"Nx\",ylab=\"Size (Kbp)\", main=\"Nx Length\")
grid(col=\"grey\")
for (i in 1:$numOfFile)
{
