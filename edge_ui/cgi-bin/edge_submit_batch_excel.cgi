...
if ($ARGV[0] && $ARGV[1] && $ARGV[2]){
	...
	$opt{"edge-batch-input-excel"} = $ARGV[1];
        $opt{'edge-qc-sw'} =1;
        $opt{'edge-qc-q'} = 5;
        $opt{'edge-qc-minl'} = 30;
        $opt{'edge-qc-avgq'} = 0;
        ...
        $opt{'edge-taxa-enabled-tools'}="gottcha2-speDB-b,pangia,centrifuge,kraken2";
        if(-e $ARGV[3]) {
...
sub readBatchInput {
	...
	if ( ! -f $excel_file){
		...
		&returnStatus();
	     ...
	     if ($excel_file !~ /xlsx$/){ 
                 open my $fh,"<", $excel_file;
		 my $header=<$fh>;
                 chomp (@output=<$fh>);	
                 close $fh;
	     }else{
                 @output = `$EDGE_HOME/bin/python $path_to_script $excel_file`;
	     }
        }
...
sub runPipeline_cluster {
	...
	foreach my $pname (@pnames){
		...
		if($error) {
			...
			`echo "Your job $job_id submitted." >> $proj_dir/process_current.log`;
			`echo "Your job $job_id submitted." >> $proj_dir/process.log`;
		}
		$proj_count++;
	}
}
...
sub getSRAmetaData{
	...
	return 1 if ($#lines == 0);
		...
		my $url2 = "https://www.ebi.ac.uk/ena/data/warehouse/search?query=accession=$sample_acc&result=sample&fields=accession,collection_date,country,description,first_public,isolation_source,location,scientific_name,sample_alias,center_name,environment_material,host,host_status,host_sex&display=report";
