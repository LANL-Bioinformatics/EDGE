$( document ).ready(function()
{
	...
	$( ".edge-file-selector" ).on( "click", function() {
		inputFileID = $(this).prevAll().children().prop("name");
	});
		...
		$.ajax({
			...
			success: function(obj){
				...
				if( $(".list-info, .list-info-delete").size() ){
					$('html, body').animate({ scrollTop: $( "#edge-submit-info" ).offset().top }, 500);
				}
			...
			success: function(obj) {
				...
				if(! $.isEmptyObject(obj.LIST)){
					...
					$.each(listIdOrder, function(i,v){
						...
						if( proj_list_obj.NAME ){
							...
							var status = proj_list_obj.STATUS;
							var desc = proj_list_obj.DESC;
				...
				if(! $.isEmptyObject(obj.PROG))
				{
					...
					$.each(listIdOrder, function(i,v){
						...
						var status = prog_list_obj.STATUS;
